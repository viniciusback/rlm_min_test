...
class RLM_REPL(RLM):
    ...
    def completion(self, context: List[str] | str | List[Dict[str, str]], query: Optional[str] = None) -> str:
        ...
        for iteration in range(self._max_iterations):
            self.logger.log(f"🔁 Iteração {iteration+1}")
            self.logger.log("🤖 Chamando agente principal...")
            ...
            self.logger.log("📨 Resposta recebida do agente")
            ...
            if code_blocks is not None and self.on_action:
                self.on_action({
                    "iteration": iteration,
                    "type": "code_execution",
                    "code": code_blocks
            ...
            if code_blocks is not None:
                self.logger.log("🧠 Chamando agente executor (REPL)...")
                self.messages = utils.process_code_execution(
                    response, self.messages, self.repl_env, 
                    self.repl_env_logger, self.logger
                )
                self.logger.log("📡 Resultado do agente executor retornado")
            ...
            if final_answer:
                self.logger.log("✅ Resposta final encontrada")
